...
int main(int argc, char * argv[])
{
  ...
  */
  ...
  // ----------------------------------------------------
  int *CtoV,*VtoC,*Codeword,*Receivedword,*Decide,*U,l;
  int iter;
  ...
  Dev_ColumnDegree = 4, Dev_RowDegree = 8;
 ...
 //  printf("\n Matrix copied ");
  ...
  printf("alpha\t\t\tNbEr(BER)\t\t\tNbFer(FER)\t\tNbtested\tIterAver(Itermax)\tNbUndec(Dmin) \t time \n");
